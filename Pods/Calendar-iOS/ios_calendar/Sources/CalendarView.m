...
- (void)setup
    {
        self.dayCellWidth = self.frame.size.width/8;  //kCalendarViewDayCellWidth;
        self.dayCellHeight = self.frame.size.height/8; //kCalendarViewDayCellHeight;
...
- (void)tap:(UITapGestureRecognizer *)recognizer
    {
        ...
        switch (type) {
            case CalendarViewTypeDay:
            {
                ...
                [self.calendarDelegate didTapDate:self.currentDate];
            }
            break;
            case CalendarViewTypeMonth:
            {
